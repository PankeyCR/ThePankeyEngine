...
void loop(){
  ...
  Serial.print("interpolateLinear "+ String(scale) +" ");Serial.println(FastMonkeyMath::interpolateLinear(scale,start,end).toNote());
  
  Serial.print("extrapolateLinear "+ String(scale) +" ");Serial.println(FastMonkeyMath::extrapolateLinear(scale,0,100));
  Serial.print("extrapolateLinear "+ String(scale) +" ");Serial.println(FastMonkeyMath::extrapolateLinear(scale,start,end).toNote());
...
//  Serial.print("log ");Serial.println(log(10,10));
  ...
  Serial.print("computeNormal ");Serial.println(FastMonkeyMath::computeNormal(c0,c1,c2).toNote());
  Serial.print("sphericalToCartesian ");Serial.println(FastMonkeyMath::sphericalToCartesian(c0).toNote());
  Serial.print("sphericalToCartesian ");Serial.println(FastMonkeyMath::sphericalToCartesian(c0).toNote());
  Serial.print("sphericalToCartesianZ ");Serial.println(FastMonkeyMath::sphericalToCartesianZ(c0).toNote());
 // Serial.print("cartesianZToSpherical ");Serial.println(FastMonkeyMath::cartesianZToSpherical(c0).toNote());//error, soon will be fix, lol
  Serial.println("end ");
}