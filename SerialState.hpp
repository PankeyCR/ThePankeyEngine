	...
	#include "SerialConnectionStateChanger.hpp"
		...
		#define SerialStateLog(location,method,type,mns) higgs_Log((void*)this,location,"SerialState",method,type,mns)
	#else
	...
	namespace higgs{
		...
		class SerialState : public SerialNetwork{
			public:
				...
				virtual ~SerialState(){
					SerialStateLog(higgs_Log_StartMethod, "Destructor", "println", "");
					SerialStateLog(higgs_Log_EndMethod, "Destructor", "println", "");
				}

				//The onConnections methods are only used if the port and the protocol are not nullptr

				virtual void onGlobalConnection(){
					SerialStateLog(higgs_Log_StartMethod, "onGlobalConnection", "println", "");
					...
					SerialStateLog(higgs_Log_EndMethod, "onGlobalConnection", "println", "");
				}

				virtual void onGlobalDisconnection(){
					SerialStateLog(higgs_Log_StartMethod, "onGlobalDisconnection", "println", "");
					...
					SerialStateLog(higgs_Log_EndMethod, "onGlobalDisconnection", "println", "");
				}

				virtual void onConnection(){
					SerialStateLog(higgs_Log_StartMethod, "onConnection", "println", "");
					...
					SerialStateLog(higgs_Log_EndMethod, "onConnection", "println", "");
				}

				virtual void onDisconnection(){
					SerialStateLog(higgs_Log_StartMethod, "onDisconnection", "println", "");
					for(int x_ports = 0; x_ports < m_ports.getPosition(); x_ports++){
						auto f_entry = m_ports.getMapEntryByPosition(x_ports);
						SerialPort* f_ports = f_entry.getKey();
						PortProtocol* f_protocol = f_entry.getValue();
					...
					SerialStateLog(higgs_Log_EndMethod, "onDisconnection", "println", "");
				}
				...
				virtual MapEntry<SerialServer,ServerProtocol> addSerialServer(SerialServer* a_serial, ServerProtocol* a_protocol){
					SerialStateLog(higgs_Log_StartMethod, "addSerialServer", "println", "");
					SerialStateLog(higgs_Log_EndMethod, "addSerialServer", "println", "");
				...
				virtual void initializeSerialServers(SerialState* a_serialState){
					SerialStateLog(higgs_Log_StartMethod, "initializeSerialServers", "println", "");
					...
					SerialStateLog(higgs_Log_EndMethod, "initializeSerialServers", "println", "");
				}

				virtual SerialServer* getSerialServer(int index){
					SerialStateLog(higgs_Log_StartMethod, "getSerialServer", "println", "int index");
					SerialStateLog(higgs_Log_EndMethod, "getSerialServer", "println", "");
				...
				virtual MapEntry<SerialServer,ServerProtocol> removeSerialServer(SerialServer* a_server){
					SerialStateLog(higgs_Log_StartMethod, "removeSerialServer", "println", "");
					SerialStateLog(higgs_Log_EndMethod, "removeSerialServer", "println", "");
					return m_servers.removeByKeyPointer(a_server);
				}

				virtual bool removeDeleteSerialServer(SerialServer* a_server){
					SerialStateLog(higgs_Log_StartMethod, "removeDeleteSerialServer", "println", "");
					SerialStateLog(higgs_Log_EndMethod, "removeDeleteSerialServer", "println", "");
				...
				virtual void removeAllSerialServer(){
					SerialStateLog(higgs_Log_StartMethod, "removeAllSerialServer", "println", "");
					m_servers.reset();
					SerialStateLog(higgs_Log_EndMethod, "removeAllSerialServer", "println", "");
				}
				...
				virtual SerialServer* getSerialServer(cppObjectClass* cls){
					SerialStateLog(higgs_Log_StartMethod, "getSerialServer", "println", "cppObjectClass* cls");
					for(int x = 0; x < m_servers.getPosition(); x++){
						SerialServer* i_server = m_servers.getKeyByPosition(x);
						if(i_server == nullptr){
							continue;
				...
				virtual MapEntry<SerialPort,PortProtocol> addSerialPort(SerialPort* a_port, PortProtocol* a_protocol){
					SerialStateLog(higgs_Log_StartMethod, "addSerialPort",  "println", "SerialPort* a_port, PortProtocol* a_protocol");
					SerialStateLog(higgs_Log_EndMethod, "addSerialPort", "println", "");
					if(a_protocol != nullptr && this->hasInitialize()){
						a_protocol->initialize(this);
					}
				...
				virtual void initializeSerialPorts(SerialState* a_serialState){
					SerialStateLog(higgs_Log_StartMethod, "initializeSerialPorts", "println", "");
					...
					SerialStateLog(higgs_Log_EndMethod, "initializeSerialPorts", "println", "");
				}
				...
				virtual bool containSerialPort(SerialPort* a_port){
					SerialStateLog(higgs_Log_StartMethod, "containSerialPort", "println", "");
					SerialStateLog(higgs_Log_EndMethod, "containSerialPort", "println", "");
				...
				virtual MapEntry<SerialPort,PortProtocol> removeSerialPort(int index){
					SerialStateLog(higgs_Log_StartMethod, "removeSerialPort",  "println", "with protocol");
					SerialStateLog(higgs_Log_EndMethod, "removeSerialPort", "println", "");
					return m_ports.removeByPosition(index);
				}

				virtual bool removeDeleteSerialPort(int index){
					SerialStateLog(higgs_Log_StartMethod, "removeDeleteSerialPort",  "println", "with protocol");
					SerialStateLog(higgs_Log_EndMethod, "removeDeleteSerialPort", "println", "");
				...
				virtual void removeAllSerialPort(){
					SerialStateLog(higgs_Log_StartMethod, "removeAllSerialPort",  "println", "with protocol");
					m_ports.reset();
					SerialStateLog(higgs_Log_EndMethod, "removeAllSerialPort", "println", "");
				}
				...
				virtual void initializeState(){
					SerialNetworkLog(higgs_Log_StartMethod, "initializeState", "println", "");
					...
					SerialNetworkLog(higgs_Log_EndMethod, "initializeState", "println", "");
				}
				
				virtual void instantSend(Note mns){
					SerialNetworkLog(higgs_Log_Statement, "instantSend",  "println", "method with Note as parameter");
					for(int x = 0; x < this->m_ports.getPosition(); x++){
						...
						SerialNetworkLog(higgs_Log_Statement, "instantSend",  "println", "trying to send port named: ");
						SerialNetworkLog(higgs_Log_Statement, "instantSend",  "println", f_ports->getName());
						f_protocol->InstantBroadcastMessage(f_ports, mns);
					}
					SerialNetworkLog(higgs_Log_EndMethod, "instantSend", "println", "");
				}

				virtual void instantSend(ByteArray a_message){}

				virtual void instantSend(int index, Note mns){
					SerialNetworkLog(higgs_Log_Statement, "instantSend",  "println", "method with int and Note as parameter");
					SerialNetworkLog(higgs_Log_Statement, "instantSend",  "println", Note("index: ") + Note(index));
					...
					if(f_ports == nullptr || i_protocol == nullptr){
						SerialNetworkLog(higgs_Log_Statement, "instantSend",  "println", "port == nullptr || protocol == nullptr");
						return;
					}
					SerialNetworkLog(higgs_Log_Statement, "instantSend",  "println", "sending port named: ");
					SerialNetworkLog(higgs_Log_Statement, "instantSend",  "println", f_ports->getName());
				...
				virtual void instantPortSend(SerialPort* a_port, Note mns){
					SerialNetworkLog(higgs_Log_Statement, "instantPortSend",  "println", "method with serialport and Note as parameter");
					SerialPort* port = a_port;
					PortProtocol* protocol = this->getPortProtocol(a_port);
					if(port == nullptr || protocol == nullptr){
						SerialNetworkLog(higgs_Log_Statement, "instantPortSend",  "println", "port == nullptr || protocol == nullptr");
						return;
					}
					protocol->InstantPrivateMessage(port, mns);
					SerialNetworkLog(higgs_Log_EndMethod, "instantPortSend", "println", "");
				}

				virtual void instantSend(Note name, Note mns){
					SerialNetworkLog(higgs_Log_Statement, "instantSend",  "println", "method with a Note and a Note as parameter");
				...
				virtual void instantByteArraySend(Note name, ByteArray array){
					SerialNetworkLog(higgs_Log_Statement, "instantByteArraySend",  "println", "method with a Note and a Note as parameter");
					SerialPort* port = this->getSerialPort(name);
					PortProtocol* portProtocol = m_ports.getValueByPointer(port);
					if(port == nullptr || portProtocol == nullptr){
						SerialNetworkLog(higgs_Log_Statement, "instantByteArraySend",  "println", "port == nullptr || protocol == nullptr");
						return;
					}
					portProtocol->InstantPrivateByteArrayMessage(port, array);
					SerialNetworkLog(higgs_Log_EndMethod, "instantByteArraySend", "println", "");
				}

				virtual void instantSend(Message mns){
					SerialNetworkLog(higgs_Log_Statement, "instantSend",  "println", "method with Message as parameter");
					for(int x = 0; x < this->m_ports.getPosition(); x++){
						...
						if(mns.id() == -1){
							SerialNetworkLog(higgs_Log_Statement, "instantSend",  "println", "portProtocols InstantBroadcastMessage");
							f_protocol->InstantBroadcastMessage(f_ports, mns.text());
						}else{
							SerialNetworkLog(higgs_Log_Statement, "instantSend",  "println", "portProtocols InstantPrivateMessage");
				...
				virtual void send(Note mns){
					SerialNetworkLog(higgs_Log_Statement, "send",  "println", "method with Note as parameter");
					m_broadMessages.addLValue(mns);
					SerialNetworkLog(higgs_Log_EndMethod, "send", "println", "");
				}

				virtual void send(ByteArray a_message){}

				virtual void send(int index, Note mns){
					SerialNetworkLog(higgs_Log_Statement, "send",  "println", "method with a int and a Note as parameters");
					SerialNetworkLog(higgs_Log_Statement, "send",  "println", Note("index: ") + Note(index));
					m_clientMessages.addPointers(new int(index), new Note(mns));
					SerialNetworkLog(higgs_Log_EndMethod, "send", "println", "");
				}

				virtual void send(Note name, Note mns){
					SerialNetworkLog(higgs_Log_Statement, "send",  "println", "method with 2 Notes as parameters");
					for(int x = 0 ; x < this->m_ports.getPosition(); x++){
						auto f_entry = m_ports.getMapEntryByPosition(x);
						...
						if(f_port->getName() == name){
							...
							SerialNetworkLog(higgs_Log_Statement, "send",  "println", higgs_String("adding"));
						}
					}
					SerialNetworkLog(higgs_Log_EndMethod, "send", "println", "");
				}
				...
				virtual void send(Message mns){
					SerialNetworkLog(higgs_Log_Statement, "send",  "println", "method with Message as parameter");
					if(mns.id() == -1){
						SerialNetworkLog(higgs_Log_Statement, "send",  "println", "no id");
						m_broadMessages.addLValue(mns.text());
					}else{
						SerialNetworkLog(higgs_Log_Statement, "send",  "println", "with id");
				...
				virtual void send(Message* mns){
					SerialNetworkLog(higgs_Log_Statement, "send",  "println", "method with Message pointer as parameter");
					if(mns->id() == -1){
						SerialNetworkLog(higgs_Log_Statement, "send",  "println", "no id");
						m_broadMessages.addLValue(mns->text());
					}else{
				...
				virtual bool isConnected(Note namae){
					SerialNetworkLog(higgs_Log_EndMethod, "isConnected", "println", "");
					for(int x = 0; x < this->m_ports.getPosition(); x++){
				...
				virtual void connect() {
					SerialNetworkLog(higgs_Log_EndMethod, "connect", "println", "");
					for(int x = 0; x < this->m_ports.getPosition(); x++){
						auto f_entry = m_ports.getMapEntryByPosition(x);
						SerialPort* f_port = f_entry.getKey();
						PortProtocol* f_protocol = f_entry.getValue();
				...
				virtual void addConnect(SerialPort* port, PortProtocol* protocol) {
					SerialNetworkLog(higgs_Log_StartMethod, "addConnect",  "println", "with protocol");
					if(port == nullptr){
						return;
					}
					...
					SerialNetworkLog(higgs_Log_EndMethod, "addConnect", "println", "");
				}

				virtual void disconect() {
					SerialNetworkLog(higgs_Log_EndMethod, "disconect", "println", "");
					for(int x = 0; x < m_ports.getPosition(); x++){
						auto f_entry = m_ports.getMapEntryByPosition(x);
						SerialPort* f_port = f_entry.getKey();
						PortProtocol* f_protocol = f_entry.getValue();
				...
				virtual void disconectDelete() {
					SerialNetworkLog(higgs_Log_EndMethod, "disconectDelete", "println", "");
					for(int x = 0; x < m_ports.getPosition(); x++){
						auto f_entry = m_ports.getMapEntryByPosition(x);
						SerialPort* f_port = f_entry.getKey();
						PortProtocol* f_protocol = f_entry.getValue();
				...
				virtual bool ping(Note note){
					SerialNetworkLog(higgs_Log_StartMethod, "ping", "println", "");
					SerialNetworkLog(higgs_Log_EndMethod, "ping", "println", "");
				...
				virtual bool instanceof(cppObjectClass* cls){
					return cls == Class<SerialState>::getClass() || SerialNetwork::instanceof(cls);
				}
				#endif
