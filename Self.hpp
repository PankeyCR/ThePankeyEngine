    ...
    #include "SelfMemoryAllocator.hpp"
		...
		#define SelfLog(location,method,type,mns) pankey_Log(this,location,"Self",method,type,mns)
	#else
		...
		class Self : public MemoryHolder<H>{
			public:
				...
				Self(SELF_TYPE* a_Self){
					SelfLog(pankey_Log_StartMethod, "Contructor", "println","const SELF_TYPE &a_Self");
					...
					SelfLog(pankey_Log_EndMethod, "Contructor", "println","");
				}

				virtual ~Self(){
					SelfLog(pankey_Log_StartMethod, "Destructor", "println","");
					this->destroy();
					SelfLog(pankey_Log_EndMethod, "Destructor", "println","");
				}
				...
				virtual SELF_TYPE& getSelf(){
					SelfLog(pankey_Log_StartMethod, "getSelf", "println","");
					VOID_TYPE i_pointer = this->getRawPointer();
					SelfLog(pankey_Log_EndMethod, "getSelf", "println","");
					return *((SELF_TYPE*)i_pointer);
				}

                virtual void setHolder(HOLDER_TYPE a_holder){
					SelfLog(pankey_Log_StartMethod, "setHolder", "println","");
					SelfLog(pankey_Log_EndMethod, "setHolder", "println","");
				}
