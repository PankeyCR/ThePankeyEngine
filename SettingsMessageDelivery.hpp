	...
	namespace pankey{
		...
		class SettingsMessageDelivery : public Command<Application&,const Note&>{
			public:
				SettingsMessageDelivery(){}
				SettingsMessageDelivery(const Note& a_sender){
					sender = a_sender;
				}
				SettingsMessageDelivery(const SettingsMessageDelivery& a_settings){
					sender = a_settings.sender;
				}
				virtual ~SettingsMessageDelivery(){}
						...
						if(i_command_1 == "Settings"){
							if(i_command_2 == "getInt"){
								if(!i_command_3.isEmpty()){
									...
									a_app.run(sender, i_command_3 + Note(" ") + Note(i_val));
								}
							}
							...
							if(i_command_2 == "getNote"){
								if(!i_command_3.isEmpty()){
									...
									a_app.run(sender, i_command_3 + Note(" ") + i_val);
								}
							}
