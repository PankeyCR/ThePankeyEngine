
#include "Note.hpp"

using namespace ame;
 
void setup() {
  Serial.begin(9600);
}

void loop() {
  ame_Debuging(ame_Log_StartLoop, "loop");

  int i_size_1 = Note::floatCharSize(1.9876f, 4);
  Serial.print("1.9876 size: ");Serial.println(i_size_1);
  char* c_value_1 = new char[i_size_1+1];
  Note::floatToCharPointer(1.9876f, i_size_1, 0, 4, c_value_1, true);
  Serial.print("value: ");Serial.println(c_value_1);
  delete[] c_value_1;

  int i_size_2 = Note::floatCharSize(12.9876f, 4);
  Serial.print("12.9876 size: ");Serial.println(i_size_2);
  char* c_value_2 = new char[i_size_2+1];
  Note::floatToCharPointer(12.9876f, i_size_2, 0, 4, c_value_2, true);
  Serial.print("value: ");Serial.println(c_value_2);
  delete[] c_value_2;

  int i_size_3 = Note::floatCharSize(123.9876f, 4);
  Serial.print("123.9876 size: ");Serial.println(i_size_3);
  char* c_value_3 = new char[i_size_3+1];
  Note::floatToCharPointer(123.9876f, i_size_3, 0, 4, c_value_3, true);
  Serial.print("value: ");Serial.println(c_value_3);
  delete[] c_value_3;

  int i_size_4 = Note::floatCharSize(1234.9876f, 4);
  Serial.print("1234.9876 size: ");Serial.println(i_size_4);
  char* c_value_4 = new char[i_size_4+1];
  Note::floatToCharPointer(1234.9876f, i_size_4, 0, 4, c_value_4, true);
  Serial.print("value: ");Serial.println(c_value_4);
  delete[] c_value_4;

  int i_size_5 = Note::floatCharSize(-1234.9876, 4);
  Serial.print("-1234.9876 size: ");Serial.println(i_size_5);
  char* c_value_5 = new char[i_size_5+1];
  Note::floatToCharPointer(-1234.9876f, i_size_5, 0, 4, c_value_5, true);
  Serial.print("value: ");Serial.println(c_value_5);
  delete[] c_value_5;
  
  ame_Debuging(ame_Log_EndLoop, "loop");
}



 
