	...
	#include "InvokeMethod.hpp"
		...
		#define InvokeRawMapLog(location,method,type,mns) pankey_Log(nullptr,location,"InvokeRawMap",method,type,mns)
	#else
	...
	namespace pankey{
		...
		void invoke(const RawMap<K,InvokeMethod<Args...>>& a_map, K a_key, Args... args){
			InvokeRawMapLog(pankey_Log_StartMethod, "invoke", "println", "");
			auto i_event = a_map.getValueByLValue(a_key);
			if(i_event == nullptr){
				InvokeRawMapLog(pankey_Log_EndMethod, "invoke", "println", "");
				return;
			}
			(**i_event)(args...);
			InvokeRawMapLog(pankey_Log_EndMethod, "invoke", "println", "");
		...
		void invokeAll(const RawMap<K,InvokeMethod<Args...>>& a_map, Args... args){
			InvokeRawMapLog(pankey_Log_StartMethod, "invokeAll", "println", "");
			for(int x = 0; x < a_map.getPosition(); x++){
				auto i_event = a_map.getValueByPosition(x);
				if(i_event == nullptr){
					InvokeRawMapLog(pankey_Log_EndMethod, "invokeAll", "println", "");
					return;
				}
				(**i_event)(args...);
			}
		...
		void invokeAll(const RawMap<K,InvokeMethodReturn<R,Args...>>& a_map, Args... args){
			InvokeRawMapLog(pankey_Log_StartMethod, "invokeAll", "println", "");
			for(int x = 0; x < a_map.getPosition(); x++){
				InvokeRawMapLog(pankey_Log_Statement, "invokeAll", "println", "loop index:");
				InvokeRawMapLog(pankey_Log_Statement, "invokeAll", "println", x);
				auto i_event = a_map.getValueByPosition(x);
				if(i_event == nullptr){
					InvokeRawMapLog(pankey_Log_EndMethod, "invokeAll", "println", "");
					continue;
