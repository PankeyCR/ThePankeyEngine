...
class Quaternion : public cppObject , public Printable{	
	...
	//	void read(JmeImporter e);
		...
		Quaternion mult(Quaternion vec, Matrix4f matrix){
			Quaternion store;
			float x = matrix.m00 * vec.x + matrix.m10 * vec.y + matrix.m20 * vec.z + matrix.m30 * vec.w;
			float y = matrix.m01 * vec.x + matrix.m11 * vec.y + matrix.m21 * vec.z + matrix.m31 * vec.w;
			float z = matrix.m02 * vec.x + matrix.m12 * vec.y + matrix.m22 * vec.z + matrix.m32 * vec.w;
			float w = matrix.m03 * vec.x + matrix.m13 * vec.y + matrix.m23 * vec.z + matrix.m33 * vec.w;
			store.x = x;
			store.y = y;
			store.z = z;
			store.w = w;
		...
		Quaternion toRotationQuat(Matrix4f matrix){
			Quaternion quat;
			quat.fromRotationMatrix(matrix.toRotationMatrix());
			return quat;
		}
		
		void toRotationQuat(Quaternion& q, Matrix4f matrix){
			q.fromRotationMatrix(matrix.toRotationMatrix());
		}
		
		// void setRotationQuaternion(Matrix4f matrix){
			// Quaternion quat;
			// matrix.toRotationMatrix(matrix);
			// set(quat);
		// }
		
		// Matrix4f multLocal(Matrix4f matrix, Quaternion rotation){
			// Vector3f axis;
