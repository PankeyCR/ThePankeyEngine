...
#include "RawMapEntry.hpp"
	...
	#define RawPointerMapLog(location,method,type,mns) higgs_Log((void*)this,location,"RawPointerMap",method,type,mns)
#else
...
class RawPointerMap{
    public:
		virtual ~RawPointerMap(){
			RawPointerMapLog(higgs_Log_StartMethod, "Constructor", "println", "");
			RawPointerMapLog(higgs_Log_EndMethod, "Constructor", "println", "");
		}

		virtual bool isEmpty()const=0;

		virtual void setOwner(bool a_owning){
			RawPointerMapLog(higgs_Log_StartMethod, "setOwner", "println", "");
			this->m_key_owner = a_owning;
			this->m_value_owner = a_owning;
			RawPointerMapLog(higgs_Log_EndMethod, "setOwner", "println", "");
		}
		virtual bool isOwner()const{
			RawPointerMapLog(higgs_Log_StartMethod, "isOwner", "println", "");
			RawPointerMapLog(higgs_Log_EndMethod, "isOwner", "println", "");
		...
		virtual void setKeyOwner(bool a_owning){
			RawPointerMapLog(higgs_Log_StartMethod, "setKeyOwner", "println", "");
			this->m_key_owner = a_owning;
			RawPointerMapLog(higgs_Log_EndMethod, "setKeyOwner", "println", "");
		}
		virtual bool isKeyOwner()const{
			RawPointerMapLog(higgs_Log_StartMethod, "isKeyOwner", "println", "");
			RawPointerMapLog(higgs_Log_EndMethod, "isKeyOwner", "println", "");
			return this->m_key_owner;
		}

		virtual void setValueOwner(bool a_owning){
			RawPointerMapLog(higgs_Log_StartMethod, "setValueOwner", "println", "");
			this->m_value_owner = a_owning;
			RawPointerMapLog(higgs_Log_EndMethod, "setValueOwner", "println", "");
		}
		virtual bool isValueOwner()const{
			RawPointerMapLog(higgs_Log_StartMethod, "isValueOwner", "println", "");
			RawPointerMapLog(higgs_Log_EndMethod, "isValueOwner", "println", "");
			return this->m_value_owner;
		}
		virtual void setPosition(int a_position){
			RawPointerMapLog(higgs_Log_StartMethod, "setPosition", "println", a_position);
			this->m_position = a_position;
			RawPointerMapLog(higgs_Log_EndMethod, "setPosition", "println", "");
		}
		virtual int getPosition()const{
			RawPointerMapLog(higgs_Log_StartMethod, "getPosition", "println", "");
			RawPointerMapLog(higgs_Log_EndMethod, "getPosition", "println", "");
			return this->m_position;
		}

		virtual void setSize(int a_size){
			RawPointerMapLog(higgs_Log_StartMethod, "setSize", "println", "");
			this->m_size = a_size;
			RawPointerMapLog(higgs_Log_EndMethod, "setSize", "println", "");
		}
		virtual int getSize()const{
			RawPointerMapLog(higgs_Log_StartMethod, "getSize", "println", "");
			RawPointerMapLog(higgs_Log_EndMethod, "getSize", "println", "");
			return this->m_size;
		}
		...
		virtual void addMove(RawPointerMap<K,V>* a_map){
			RawPointerMapLog(higgs_Log_StartMethod, "addMove", "println", "");
			...
			RawPointerMapLog(higgs_Log_EndMethod, "addMove", "println", "");
		}

		virtual void addDuplicate(RawPointerMap<K,V>* a_map){
			RawPointerMapLog(higgs_Log_StartMethod, "addDuplicate", "println", "");
			for(int x = 0; x < a_map->getPosition(); x++){
				K* k = a_map->getKeyByPosition(x);
				V* v = a_map->getValueByPosition(x);
				this->addPointers(k,v);
			}
			RawPointerMapLog(higgs_Log_EndMethod, "addDuplicate", "println", "");
		}
		...
		virtual RawMapEntry<K,V> putPointers(K* a_key, V* a_value){
			RawPointerMapLog(higgs_Log_StartMethod, "putPointers", "println", "");
			if(this->containPairPointers(a_key,a_value)){
				RawPointerMapLog(higgs_Log_EndMethod, "putPointers", "println", "");
		...
		virtual RawMapEntry<K,V> putRawMapEntry(RawMapEntry<K,V> a_map_entry){
			RawPointerMapLog(higgs_Log_StartMethod, "putRawMapEntry", "println", "");
			if(this->containRawMapEntry(a_map_entry)){
				RawPointerMapLog(higgs_Log_EndMethod, "putRawMapEntry", "println", "");
		...
		virtual bool removeDeleteByKeyPointer(K* a_key){
			RawPointerMapLog(higgs_Log_StartMethod, "removeDeleteByKeyPointer", "println", "");
			RawMapEntry<K,V> i_entry = this->removeByKeyPointer(a_key);
			bool removed = !i_entry.isNull();
			if(removed && this->isOwner()){
				i_entry.deleteEntry();
			}
			RawPointerMapLog(higgs_Log_EndMethod, "removeDeleteByKeyPointer", "println", "");
			return removed;
		}

		virtual bool removeDeleteByValuePointer(V* a_value){
			RawPointerMapLog(higgs_Log_StartMethod, "removeDeleteByValuePointer", "println", "");
			RawMapEntry<K,V> i_entry = this->removeByValuePointer(a_value);
			bool removed = !i_entry.isNull();
			if(removed && this->isOwner()){
				i_entry.deleteEntry();
			}
			RawPointerMapLog(higgs_Log_EndMethod, "removeDeleteByValuePointer", "println", "");
			return removed;
		}

		virtual bool removeDeleteByPosition(int a_position){
			RawPointerMapLog(higgs_Log_StartMethod, "removeDeleteByPosition", "println", "");
			RawMapEntry<K,V> i_entry = this->removeByPosition(a_position);
			bool removed = !i_entry.isNull();
			if(removed && isOwner()){
				i_entry.deleteEntry();
			}
			RawPointerMapLog(higgs_Log_EndMethod, "removeDeleteByPosition", "println", "");
			return removed;
		}

		virtual int getKeyIndexByPointer(K* a_key){
			RawPointerMapLog(higgs_Log_StartMethod, "getKeyIndexByPointer", "println", "");
			if(this->isEmpty()){
				RawPointerMapLog(higgs_Log_EndMethod, "getKeyIndexByPointer", "println", "");
				return -1;
			}
		...
		virtual int getValueIndexByPointer(V* a_value){
			RawPointerMapLog(higgs_Log_StartMethod, "getValueIndexByPointer", "println", "");
			if(this->isEmpty()){
				RawPointerMapLog(higgs_Log_EndMethod, "getValueIndexByPointer", "println", "");
				return -1;
			}
			for(int x = 0; x < this->getPosition(); x++){
	...
	protected:
		...
		virtual void incrementPosition(){
			RawPointerMapLog(higgs_Log_StartMethod, "incrementPosition", "println", this->m_position);
			this->m_position++;
			RawPointerMapLog(higgs_Log_EndMethod, "incrementPosition", "println", this->m_position);
		}
		virtual void decrementPosition(){
			RawPointerMapLog(higgs_Log_StartMethod, "decrementPosition", "println", this->m_position);
			this->m_position--;
			if(this->m_position < 0){
				this->m_position = 0;
			}
			RawPointerMapLog(higgs_Log_EndMethod, "decrementPosition", "println", this->m_position);
		}
