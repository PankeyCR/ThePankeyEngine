...
#include <EthernetUdp.h>
	...
	#define UDPNetworkLog(location,method,type,mns) pankey_Log(this,location,"UDPNetwork",method,type,mns)
#else
	#ifdef UDPNetwork_LogDebugApp
		#include "pankey_Logger_config.hpp"
		#include "pankey_Logger.hpp"
		
		#define UDPNetworkLog(location,method,type,mns) pankey_LogDebug(this,location,"UDPNetwork",method,type)
	#else
...
class UDPNetwork : public SerialNetwork{	
    public:	
		...
		virtual void initializeState(){
			UDPNetworkLog(pankey_Log_StartMethod, "initializeState", "println", "");
			UDPNetworkLog(pankey_Log_Statement, "initializeState", "println", "Port:");
			UDPNetworkLog(pankey_Log_Statement, "initializeState", "println", m_port);
			SerialNetwork::initializeState();
			Udp.begin(m_port);
			UDPNetworkLog(pankey_Log_EndMethod, "initializeState", "println", "");
		}
		
		virtual void updateState(float tpc){
			//UDPNetworkLog(pankey_Log_StartMethod, "updateState", "println", "");
			int packetSize = Udp.parsePacket();
			if (packetSize) {
				UDPNetworkLog(pankey_Log_Statement, "updateState", "println", "packetSize");
				UDPNetworkLog(pankey_Log_Statement, "updateState", "println", packetSize);
				IPAddress i_remote = Udp.remoteIP();
				int i_port = Udp.remotePort();
				int i_id = this->getPortId(i_remote);
				if(i_id == -1){
					UDPNetworkLog(pankey_Log_EndMethod, "updateState", "println", "i_id == -1");
					return;
				}
				...
				UDPNetworkLog(pankey_Log_EndMethod, "updateState", "println", "packetSize");
			}
			//UDPNetworkLog(pankey_Log_EndMethod, "updateState", "println", "");
		}
