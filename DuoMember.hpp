	...
	#include "Member.hpp"
		...
		#define DuoMemberLog(location,method,type,mns) higgs_Log((void*)this,location,"DuoMember",method,type,mns)
	#else
	...
	namespace higgs{
		...
		class DuoMember : public Member<H,M>{
			public:
				...
				DuoMember(){
					DuoMemberLog(higgs_Log_StartMethod, "Constructor", "println", "");
					DuoMemberLog(higgs_Log_EndMethod, "Constructor", "println", "");
				}
				...
				virtual ~DuoMember(){
					DuoMemberLog(higgs_Log_StartMethod, "Destructor", "println", "");
					this->removeDuo();
					DuoMemberLog(higgs_Log_EndMethod, "Destructor", "println", "");
				}

                virtual void copyDuoMember(const Member<H,M>& a_holder){
                    DuoMemberLog(higgs_Log_StartMethod, "copyDuoMember", "println","");
                    this->setDuoHolder(a_holder.getHolder());
                    DuoMemberLog(higgs_Log_EndMethod, "copyDuoMember", "println","");
                }

                virtual Member<H,M> getDuoMember(){
                    DuoMemberLog(higgs_Log_StartMethod, "getDuoMember", "println","");
                    Member<H,M> i_member;
                    i_member.setHolder(this->m_duo_holder);
                    DuoMemberLog(higgs_Log_EndMethod, "getDuoMember", "println","");
                    return i_member;
                }

                virtual void removeDuo(){
                    DuoMemberLog(higgs_Log_StartMethod, "remove", "println","");
                    MemoryHolderManager<H>::remove(this->getManager(), this->m_duo_holder);
                    DuoMemberLog(higgs_Log_EndMethod, "remove", "println","");
                }

                virtual void destroyDuo(){
                    DuoMemberLog(higgs_Log_StartMethod, "destroy", "println","");
                    MemoryHolderManager<H>::destroy(this->getManager(), this->m_duo_holder);
                    DuoMemberLog(higgs_Log_EndMethod, "destroy", "println","");
                }
                    
                //unsafe method
                virtual void setDuoHolder(HOLDER_TYPE a_holder){
                    DuoMemberLog(higgs_Log_StartMethod, "setDuoHolder", "println","");
					MemoryHolderManager<H>::add(this->getManager(), a_holder);
					MemoryHolderManager<H>::remove(this->getManager(), this->m_duo_holder);
					this->m_duo_holder = a_holder;
                    DuoMemberLog(higgs_Log_EndMethod, "setDuoHolder", "println","");
                }
                ...
                virtual void copyDuoMemoryHolder(const MemoryHolder<H>& a_holder){
                    DuoMemberLog(higgs_Log_StartMethod, "copyDuoMemoryHolder", "println","");
                    if(!this->isDuoMember(a_holder)){
                        DuoMemberLog(higgs_Log_Error, "copyDuoMemoryHolder", "println", "Variable is not Member of Variable");
                        DuoMemberLog(higgs_Log_EndMethod, "copyDuoMemoryHolder", "println","");
                        return;
                    }
                    this->setDuoHolder(a_holder.getHolder());
                    DuoMemberLog(higgs_Log_EndMethod, "copyDuoMemoryHolder", "println","");
				...
				virtual bool sameDuoType(long a_type)const{
                    DuoMemberLog(higgs_Log_StartMethod, "sameDuoType", "println","");
                    if(a_type == -1){
                        DuoMemberLog(higgs_Log_Error, "sameDuoType", "println","The variable cheching has no type");
                        DuoMemberLog(higgs_Log_EndMethod, "sameDuoType", "println","");
                        return false;
                    }
                    long i_type = this->getDuoType();
                    if(i_type != a_type && i_type != -1){
                        DuoMemberLog(higgs_Log_Error, "sameDuoType", "println","Not a memeber because it has diferent types");
                        DuoMemberLog(higgs_Log_EndMethod, "sameDuoType", "println","");
                        return false;
                    }
                    DuoMemberLog(higgs_Log_EndMethod, "sameDuoType", "println","");
				...
				virtual bool sameDuoBaseType(long a_type)const{
                    DuoMemberLog(higgs_Log_StartMethod, "sameDuoBaseType", "println","");
                    if(a_type == -1){
                        DuoMemberLog(higgs_Log_Error, "sameDuoBaseType", "println","The variable cheching has no type");
                        DuoMemberLog(higgs_Log_EndMethod, "sameDuoBaseType", "println","");
                        return false;
                    }
                    long i_type = this->getDuoBaseType();
                    if(i_type != a_type && i_type != -1){
                        DuoMemberLog(higgs_Log_Error, "sameDuoBaseType", "println","Not a memeber because it has diferent types");
                        DuoMemberLog(higgs_Log_EndMethod, "sameDuoBaseType", "println","");
                        return false;
                    }
                    DuoMemberLog(higgs_Log_EndMethod, "sameDuoBaseType", "println","");
				...
				virtual bool isDuoMember(MANAGER_TYPE a_manager, long a_type)const{
                    DuoMemberLog(higgs_Log_StartMethod, "isDuoMember", "println","");
                    DuoMemberLog(higgs_Log_EndMethod, "isDuoMember", "println","");
				...
				virtual void setDuoType(long a_type){
                    DuoMemberLog(higgs_Log_StartMethod, "setDuoType", "println","");
					MemoryHolderManager<H>::setType(this->getManager(), this->m_duo_holder, a_type);
                    DuoMemberLog(higgs_Log_EndMethod, "setDuoType", "println","");
				}

				virtual long getDuoType()const{
                    DuoMemberLog(higgs_Log_StartMethod, "getDuoType", "println","");
                    DuoMemberLog(higgs_Log_EndMethod, "getDuoType", "println","");
					return MemoryHolderManager<H>::getType(this->getManager(), this->m_duo_holder);
				}

				virtual void setDuoBaseType(long a_type){
                    DuoMemberLog(higgs_Log_StartMethod, "setDuoBaseType", "println","");
					MemoryHolderManager<H>::setBaseType(this->getManager(), this->m_duo_holder, a_type);
                    DuoMemberLog(higgs_Log_EndMethod, "setDuoBaseType", "println","");
				}

				virtual long getDuoBaseType()const{
                    DuoMemberLog(higgs_Log_StartMethod, "getDuoBaseType", "println","");
                    DuoMemberLog(higgs_Log_EndMethod, "getDuoBaseType", "println","");
					return MemoryHolderManager<H>::getBaseType(this->getManager(), this->m_duo_holder);
				}

				virtual bool isDuoMemoryNull()const{
					DuoMemberLog(higgs_Log_StartMethod, "isDuoMemoryNull", "println","");
					if(this->m_duo_holder == nullptr){
						DuoMemberLog(higgs_Log_EndMethod, "isDuoMemoryNull", "println","this->m_duo_holder == nullptr");
						return true;
					}
					VOID_TYPE i_pointer = MemoryHolderManager<H>::get(this->getManager(), this->m_duo_holder);
					DuoMemberLog(higgs_Log_Statement, "isDuoMemoryNull", "println", "isDuoNull: ");
					DuoMemberLog(higgs_Log_Statement, "isDuoMemoryNull", "println", i_pointer == nullptr);
					DuoMemberLog(higgs_Log_EndMethod, "isDuoMemoryNull", "println","");
					return i_pointer == nullptr;
				}

				//unsafe method
				virtual VOID_TYPE getDuoRawPointer()const{
                    DuoMemberLog(higgs_Log_StartMethod, "getDuoRawPointer", "println","");
					VOID_TYPE i_pointer = MemoryHolderManager<H>::get(this->getManager(), this->m_duo_holder);
					DuoMemberLog(higgs_Log_Statement, "getDuoRawPointer", "println","VOID_TYPE is null?");
					DuoMemberLog(higgs_Log_Statement, "getDuoRawPointer", "println", i_pointer == nullptr);
					DuoMemberLog(higgs_Log_EndMethod, "getDuoRawPointer", "println","");
					return i_pointer;
				}
				
				virtual DuoMember& operator=(const DuoMember<H,M>& a_duo_member){
					DuoMemberLog(higgs_Log_StartMethod, "operator=", "println", "");
                    this->setDuoHolder(a_duo_member.getDuoHolder());
                    this->setHolder(a_duo_member.getHolder());
					DuoMemberLog(higgs_Log_EndMethod, "operator=", "println", "");
					return *this;
				}
				...
				virtual bool isNull()const{
					DuoMemberLog(higgs_Log_StartMethod, "isDuoMemberNull", "println", "");
					DuoMemberLog(higgs_Log_EndMethod, "isDuoMemberNull", "println", "");
