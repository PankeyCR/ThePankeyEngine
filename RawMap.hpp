...
#include "RawPointerMap.hpp"
	...
	#define RawMapLog(location,method,type,mns) higgs_Log((void*)this,location,"RawMap",method,type,mns)
#else
...
class RawMap : virtual public RawPointerMap<K,V>{
    public:
		virtual ~RawMap(){
			RawMapLog(higgs_Log_StartMethod, "Destructor", "println", "");
			RawMapLog(higgs_Log_EndMethod, "Destructor", "println", "");
		}

		virtual void addCopy(const RawPointerMap<K,V>& a_map){
			RawMapLog(higgs_Log_StartMethod, "addCopy", "println", "");
			...
			RawMapLog(higgs_Log_EndMethod, "addCopy", "println", "");
		}
		...
		virtual RawMapEntry<K,V> add(K a_key, V a_value){
			RawMapLog(higgs_Log_StartMethod, "add", "println", "");
			RawMapLog(higgs_Log_EndMethod, "add", "println", "");
		...
		virtual RawMapEntry<K,V> set(K a_key, V a_value){
			RawMapLog(higgs_Log_StartMethod, "set", "println", "");
			RawMapLog(higgs_Log_EndMethod, "set", "println", "");
		...
		virtual bool contain(K a_key){
			RawMapLog(higgs_Log_StartMethod, "contain", "println", "");
			RawMapLog(higgs_Log_EndMethod, "contain", "println", "");
		...
		virtual V get(K a_key)const{
			RawMapLog(higgs_Log_StartMethod, "get", "println", "");
			V* i_value = this->getValueByLValue(a_key);
			if(i_value == nullptr){
				RawMapLog(higgs_Log_EndMethod, "get", "println", "");
				return V();
			}
			RawMapLog(higgs_Log_EndMethod, "get", "println", "");
			return *i_value;
		...
		virtual K getKey(int a_position){
			RawMapLog(higgs_Log_StartMethod, "getKey", "println", "");
			K* i_key = this->getKeyByPosition(a_position);
			if(i_key == nullptr){
				RawMapLog(higgs_Log_EndMethod, "getKey", "println", "");
				return K();
			}
			RawMapLog(higgs_Log_EndMethod, "getKey", "println", "");
			return *i_key;
		...
		virtual V getValue(int a_position){
			RawMapLog(higgs_Log_StartMethod, "getValue", "println", "");
			V* i_value = this->getValueByPosition(a_position);
			if(i_value == nullptr){
				RawMapLog(higgs_Log_EndMethod, "getValue", "println", "");
				return V();
			}
			RawMapLog(higgs_Log_EndMethod, "getValue", "println", "");
			return *i_value;
		...
		virtual RawMapEntry<K,V> put(K a_key, V a_value){
			RawMapLog(higgs_Log_StartMethod, "put", "println", "");
			RawMapLog(higgs_Log_EndMethod, "put", "println", "");
		...
		void addKeyPack(Args... a_values){
			RawMapLog(higgs_Log_StartMethod, "addKeyPack", "println", "");
			K i_array[] = {a_values...};
			for(const K& k : i_array){
				this->addPointers(new K(k), new V());
			}
			RawMapLog(higgs_Log_EndMethod, "addKeyPack", "println", "");
		}
		
		template<class... Args>
		void addValuePack(Args... a_values){
			RawMapLog(higgs_Log_StartMethod, "addValuePack", "println", "");
			V i_array[] = {a_values...};
			for(const V& v : i_array){
				this->addPointers(new K(), new V(v));
			}
			RawMapLog(higgs_Log_EndMethod, "addValuePack", "println", "");
		}
