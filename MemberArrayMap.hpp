		...
		class MemberArrayMap : public MemberArrayMapCollection<H,M>{
			public:
				...
				virtual void move(Collection<H,M>& a_collection){
					MemberArrayMapLog(pankey_Log_StartMethod, "move", "println", "");
					// for(int x = 0; x < a_collection.length(); x++){
					// 	Member<H,M> f_value = a_collection.get(x);
					// 	this->add(f_value);
					// }
					// a_collection.clear();
					MemberArrayMapLog(pankey_Log_EndMethod, "move", "println", "");
				}
				
				virtual void duplicate(const Collection<H,M>& a_collection){
					MemberArrayMapLog(pankey_Log_StartMethod, "duplicate", "println", "");
					// for(int x = 0; x < a_collection.length(); x++){
					// 	Member<H,M> f_value = a_collection.get(x);
					// 	this->add(f_value);
					// }
					MemberArrayMapLog(pankey_Log_EndMethod, "duplicate", "println", "");
				}
		};
